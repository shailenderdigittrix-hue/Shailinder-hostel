        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body p-0">
                    <div class="card-body">
                        <form action="{{ route('student-leaves.update', $leave->id) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="student_id" id="student_id" class="form-select" required>
                                    ...
                                    @foreach($students as $student)
                                        <option value="{{ $student->id }}" {{ (old('student_id', $leave->student_id) == $student->id) ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" id="reason" name="reason" class="form-control" required value="{{ old('reason', $leave->reason) }}">
                                <div class="invalid-feedback">Please enter a reason for leave.</div>
                            </div>

                            <div class="mb-3">
                                <label for="remarks" class="form-label">Remarks (optional)</label>
                                <textarea id="remarks" name="remarks" class="form-control" rows="3">{{ old('remarks', $leave->remarks) }}</textarea>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if ($leave->document)
                                    <small class="form-text text-muted">
                                        Current file: <a href="{{ asset('storage/' . $leave->document) }}" target="_blank">View Document</a>
                                    </small>
                                @endif
                            </div>
